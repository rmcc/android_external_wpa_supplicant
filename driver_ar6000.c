...
int ar6000_priv_driver_cmd(void *priv, char *cmd, char *buf, size_t buf_len)
{
        struct wpa_driver_wext_data *drv = priv;
  	struct ifreq ifr;
   	strncpy(ifr.ifr_name, drv->ifname, sizeof(ifr.ifr_name));

	wpa_printf(MSG_DEBUG, "calling atheros priv driver with cmd %s for %s\n",cmd,drv->ifname);

        if (os_strcasecmp(cmd, "RSSI") == 0) {
		((int *)buf)[0] = AR6000_XIOCTL_ANDROID_GET_RSSI;
   		ifr.ifr_data = buf;
   		int ret = ioctl(drv->ioctl_sock, AR6000_IOCTL_EXTENDED, &ifr);
		if (ret<0) 
			wpa_printf(MSG_DEBUG, "rssi ioctl error was %s\n",strerror(errno));
		else
			sprintf(ifr.ifr_data,"SSID rssi %i",((int *)(ifr.ifr_data))[0]);
		return ret;
        } else if (os_strcasecmp(cmd, "LINKSPEED") == 0) {
		((int *)buf)[0] = AR6000_XIOCTL_ANDROID_GET_LINKSPEED;
   		ifr.ifr_data = buf;
   		int ret = ioctl(drv->ioctl_sock, AR6000_IOCTL_EXTENDED, &ifr);
		
		if (ret<0) 
			wpa_printf(MSG_DEBUG, "linkspeed ioctl error was %s\n",strerror(errno));
		else
			sprintf(ifr.ifr_data,"LinkSpeed %i",((int *)(ifr.ifr_data))[0]);
        	return ret;
        /*} else if (os_strncasecmp(cmd, "COUNTRY",6) == 0) {
		((int *)buf)[0] = AR6000_XIOCTL_AP_SET_COUNTRY;
		int country = 840; // US
		if (os_strcasecmp(cmd+8,"EU")) {
			country = 826; // UK
		} else if (os_strcasecmp(cmd+8,"JP")) {
			country = 392; // JP
		}
		((char *)buf)[1] = country;
   		ifr.ifr_data = buf;
	...
	wpa_printf(MSG_DEBUG, "fallback to generic extended invocation");
        //return set80211priv(drv,AR6000_IOCTL_EXTENDED,cmd,buf_len,0);
        return 0;
}
...
#endif
#ifdef ANDROID
        .driver_cmd = ar6000_priv_driver_cmd,
#endif
};
